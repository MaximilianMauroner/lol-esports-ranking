...
export function executeDurableGc(options: { store: DurableObjectStore; plan: Record<string, unknown> & { safe: boolean; plannedDeletes: Array<{ key: string; bytes: number }>; reason?: string }; dryRun?: boolean; guard?: { authorityKey: string; verify: () => Promise<boolean | { valid: boolean; reason?: string }> }; beforeDelete?: (entry: { key: string; bytes: number }) => Promise<void> }): Promise<Record<string, unknown>>

...
test('browser data artifact stays compact and does not ship the full snapshot', async () => {
  ...
  assert.equal(playerDirectory.players?.every((player) => player.diagnostics?.sourceProvider === 'oracles-elixir'), true)
  assert.equal(playerDirectory.players?.every((player) => player.diagnostics?.scope === 'rated-complete-role-matchups'), true)
  assert.equal(playerDirectory.players?.every((player) => typeof player.diagnostics?.sampleGames === 'number'), true)
  assert.equal(playerDirectory.players?.every((player) => typeof player.diagnostics?.damageShare?.games === 'number'), true)
  ...
  assert.equal(playerDirectory.players?.every((player) => typeof player.individualResidual?.adjustedSameRoleDiff?.games === 'number'), true)

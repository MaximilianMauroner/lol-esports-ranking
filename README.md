...
Raw and private-state objects are globally content-addressed and uploaded create-only with digest verification. Raw uploads request an S3-managed SHA-256 checksum. Maintenance remains metadata-only for raw payloads: it validates exact keys, categories, digests, existence, byte lengths, and independently returned checksums through `HEAD`. When an S3-compatible bucket cannot return a stored checksum, maintenance reports `integrityDeferred` with reason `checksum-unavailable` and leaves byte-level verification to cold restore; it never downloads raw payloads while planning or replanning garbage collection. Cold restore downloads and hashes every referenced payload, and any mismatch forces a clean bootstrap instead of accepting the checkpoint. User metadata is never treated as integrity proof. The active pointer references exact raw and private manifests, so interrupted uploads remain unreachable and stale workers cannot replace a successor's raw baseline. Retention keeps the active manifest, the latest valid manifest per recent UTC day, and only pointer-authorized permanent month-end/season/international boundaries. Newly staged unreachable objects receive a bounded grace period before collection; missing or corrupt reachability data fails the entire sweep closed.

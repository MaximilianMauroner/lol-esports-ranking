...
The active pointer may also carry a `previousGeneration` authority. On a generation change it captures the prior public manifest and, when present as complete pairs, its state-manifest and raw-receipt authorities. Re-publishing the same active generation preserves the existing rollback target.

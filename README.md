...
Deployment cutover is deliberately narrow: while the active pointer still names a pre-change schema-v1 content-addressed manifest and omits `publicManifestSchemaVersion`, the server and incremental restore path may normalize that manifest to schema v2 in memory only when every current object mapping and storage/transport encoding field is already exact. A pointer marked `publicManifestSchemaVersion: 2` requires a native schema-v2 manifest. The stored manifest is never rewritten, GC and rollback do not accept it, and the compatibility ends automatically when the first native v2 generation is promoted. That first promotion intentionally omits `previousGeneration`, because the pre-change authority is not a usable v2 rollback target.

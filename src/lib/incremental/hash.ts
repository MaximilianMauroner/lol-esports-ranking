...
export function stableSerialize(value: unknown): string {
  ...
  function serialize(entry: unknown, inArray: boolean): string | undefined {
    ...
    if (typeof entry === 'bigint') throw new TypeError('Cannot stable-serialize a BigInt value')
    if (typeof entry !== 'object') return inArray ? 'null' : undefined
    if (ancestors.has(entry)) throw new TypeError('Cannot stable-serialize a cyclic value')
    ...
    try {
      ...
      const properties = Object.keys(entry)
        .sort()
        .flatMap((key) => {
          const serialized = serialize((entry as Record<string, unknown>)[key], false)
...
/** Browser-safe synchronous SHA-256 used for private compatibility state. */
